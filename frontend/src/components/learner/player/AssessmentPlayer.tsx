...
import {
  ...
  ChevronRight, RotateCcw, AlertCircle, CheckCircle2,
...
export const AssessmentPlayer = ({
  ...
  // Fetch and display the result of the last completed attempt without starting a new one
  const handleViewResults = useCallback(async () => {
    if (!assessment?.last_attempt_id) return;
    const resultData = await playerApi.getAttemptResult(assessment.last_attempt_id);
  ...
  // True only when the last attempt actually passed — not just lesson completion
  const hasPassed  = assessment.last_result_status === 'PASS';
  const hasFailed  = assessment.last_result_status === 'FAIL';
  const isPending  = assessment.last_result_status === 'PENDING';
      ...
      <div className="flex flex-col h-full">
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-lg">
            ...
            {/* Failed — can retake or no attempts left */}
            {hasFailed && (
              <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-md px-3 py-2.5 mb-4">
                ...
                <p className="text-sm font-medium text-red-700">
                  {assessment.attempts_remaining > 0
                    ? `You did not pass. ${assessment.attempts_remaining} attempt${assessment.attempts_remaining !== 1 ? 's' : ''} remaining.`
                    : 'You did not pass and have no attempts remaining. Contact your instructor to request a retake.'}
                </p>
              </div>
            )}
            ...
            {isPending && (
              <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-md px-3 py-2.5 mb-4">
                <Clock className="h-4 w-4 text-amber-500 flex-shrink-0" />
                <p className="text-sm font-medium text-amber-700">
                  Your last submission is awaiting instructor review.
                </p>
              </div>
            )}

            {/* Attempts exhausted (no prior result yet) */}
            {attemptsExhausted && !hasFailed && !hasPassed && !isPending && (
              <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-md px-3 py-2.5 mb-4">
                <XCircle className="h-4 w-4 text-red-500 flex-shrink-0" />
                <p className="text-sm font-medium text-red-700">
            ...
            {/* Start / Review button */}
            <button
              onClick={hasPassed || (hasFailed && assessment.attempts_remaining === 0)
                ? handleViewResults
                : handleStartQuiz}
              disabled={
                startAttemptMutation.isPending ||
                isPending ||
                (!hasPassed && !hasFailed && attemptsExhausted) ||
                (hasFailed && assessment.attempts_remaining === 0 && !assessment.last_attempt_id)
              }
            ...
            >
              {startAttemptMutation.isPending
                ...
                : hasPassed
                ? 'Review Results'
                : hasFailed && assessment.attempts_remaining === 0
                ? 'View Last Result'
                : hasFailed && assessment.attempts_remaining > 0
                ? 'Retake Quiz'
                : 'Start Quiz'}
          ...
          <div className="max-w-2xl mx-auto px-6 py-8">
            ...
            {result.instructor_feedback && (
              <div className="bg-blue-50 border border-blue-200 rounded-md p-4 mb-6">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{result.instructor_feedback}</p>
              </div>
            )}
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-col gap-2">
                  {result.answers.map((answer, idx) => (
                    <QuestionReviewCard key={answer.question_id} answer={answer} index={idx} />
                  ))}
                </div>
              </div>
            )}
            ...
            {!isPassed && !isPending && assessment.attempts_remaining > 0 && (
              <button onClick={handleRetry} className="btn btn-secondary w-full mb-2">
            ...
            {!isPassed && !isPending && assessment.attempts_remaining === 0 && (
              <p className="text-center text-xs text-gray-400 mb-2">
                No attempts remaining. Contact your instructor to request a retake.
              </p>
            )}
          </div>
        </div>
...
// ── QuestionReviewCard — Coursera-style per-question result ───────────────────
...
function QuestionReviewCard({ answer, index }: { answer: AttemptResultAnswer; index: number }) {
  ...
  return (
    <div className="border border-gray-200 rounded-md overflow-hidden">
      ...
      >
        <span className={cn('flex items-center gap-1.5 text-xs font-semibold', headerColor)}>
          {headerIcon}
          {statusLabel}
        </span>
        <span className="flex-1 text-xs text-gray-500 truncate">
          Q{index + 1}. {answer.question_text.slice(0, 80)}{answer.question_text.length > 80 ? '…' : ''}
        </span>
        {open
      ...
      {open && (
        <div className="px-4 pb-4 pt-1 border-t border-gray-100 bg-gray-50">
          ...
          {answer.scenario_text && (
            <div className="bg-blue-50 border border-blue-200 rounded-md p-3 mb-3 text-sm text-gray-700 leading-relaxed">
              {answer.scenario_text}
            </div>
          )}

          {/* Question */}
          <p className="text-sm font-medium text-gray-900 mb-3 leading-snug">{answer.question_text}</p>

          {/* Pending review — no answer reveal */}
          {isPending && (
          ...
          {/* Auto-graded with options */}
          {answer.is_auto_graded && answer.correct_options.length > 0 && (
            <div className="flex flex-col gap-1.5">
              {/* Show all options with correct/selected indicators */}
              {answer.correct_options.map(opt => {
                const wasSelected = answer.selected_options.some(s => s.id === opt.id);
                return (
                  <div key={opt.id} className={cn(
                    ...
                    {wasSelected && <span className="text-xs font-semibold">Your answer ✓</span>}
                    {!wasSelected && <span className="text-xs text-emerald-600">Correct answer</span>}
                  </div>
                );
              })}
              {/* Wrong selections not in correct options */}
              {answer.selected_options
                .filter(s => !answer.correct_options.some(c => c.id === s.id))
                .map(opt => (
                  <div key={opt.id} className="flex items-center gap-2 px-3 py-2 rounded-md text-sm border bg-red-50 border-red-200 text-red-800">
                    <XCircle className="h-3.5 w-3.5 flex-shrink-0" />
                    <span className="flex-1">{opt.option_text}</span>
          ...
          {/* Descriptive / text answer — show after grading */}
          {answer.is_auto_graded && answer.answer_text && answer.correct_options.length === 0 && (
            <div className="bg-white border border-gray-200 rounded-md p-3 text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">
              {answer.answer_text}
            </div>
          )}
          ...
          {answer.explanation_text && (
            <div className="mt-3 bg-blue-50 border border-blue-100 rounded-md p-3">
              <p className="text-[10px] font-semibold uppercase tracking-wide text-blue-400 mb-1">Explanation</p>
              <p className="text-xs text-gray-600 leading-relaxed">{answer.explanation_text}</p>
            </div>
          )}

          {/* Skipped */}
          {isSkipped && (
            <p className="text-xs text-gray-400 italic">This question was not attempted.</p>
          )}
        </div>
      )}
    </div>
